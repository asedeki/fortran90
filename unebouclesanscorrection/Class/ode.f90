module ode
contains
  subroutine ode_rkf ( f, neqn, y, t, tout, relerr, abserr, iflag, work, iwork )
...
!  Reference:
!
!    Lawrence Shampine and Marilyn Gordon,
...
!
!    Input/output, integer IFLAG, indicates the status of integration.
...
!
!    Input, real ( kind = 8 ) X, a "working copy" of T, the current value 
!    of the independent variable, which is adjusted as the code attempts
!    to take a step.
!
!    Input, real ( kind = 8 ) H, the current stepsize.
!
!    Input, real ( kind = 8 ) HOLD, the last successful stepsize.
!
!    Input/output, logical START, is TRUE on input for the first step.
!    The program initializes data, and sets START to FALSE.
!
!    Workspace, real ( kind = 8 ) TOLD, the previous value of T.
!
!    Workspace, real ( kind = 8 ) DELSGN, the sign (+1 or -1) of
!    TOUT - T.
!
!    Input/output, integer NS, the number of steps taken with stepsize H.
!
!    Input, integer NORND, ?
!
!    Input, integer K, the order of the current ODE method.
!
!    Input, integer KOLD, the order of the ODE method on the previous step.
!
!    Input/output, integer ISNOLD, the previous value of ISN, the sign
!    of IFLAG.
!
!  Local parameters:
!
...
!
!    Input, real ( kind = 8 ) HOLD, the step size used on the last 
!    successful step.
!
!    Input, integer K, the appropriate order for the next step.
!
!    Input/output, integer KOLD, the order used on the last successful step.
!
...
!    Input, integer NEQN, the number of equations.
!
!    Input, integer KOLD, the order used for the last successful step.
!
