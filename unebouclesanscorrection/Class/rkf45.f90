...
!
  ...
  real ( kind = 8 ) ch
  external f
  real ( kind = wp ) f1(neqn)
  real ( kind = wp ) f2(neqn)
  real ( kind = wp ) f3(neqn)
  real ( kind = wp ) f4(neqn)
  real ( kind = wp ) f5(neqn)
  real ( kind = wp ) h
  real ( kind = wp ) s(neqn)
  real ( kind = wp ) t
  real ( kind = wp ) y(neqn)
  real ( kind = wp ) yp(neqn)
...
subroutine fehl_s ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, s )
...
subroutine rkf45_d ( f, neqn, y, yp, t, tout, relerr, abserr, flag )
 use parameters,only:wp
!*******************************************************************************
!
...
!
  ...
  real ( kind = wp ) abserr
  real ( kind = wp ), save :: abserr_save = -1.0D+00
!$OMP THREADPRIVATE(abserr_save)
  real ( kind = wp ) ae
  real ( kind = wp ) dt
  real ( kind = wp ) ee
  real ( kind = wp ) eeoet
...
!$OMP THREADPRIVATE(h)
  ...
  logical output
  real ( kind = wp ) relerr
  real ( kind = wp ) relerr_min
  real ( kind = wp ), save :: relerr_save = -1.0D+00
  real ( kind = wp ), parameter :: remin = 1.0E-12
!$OMP THREADPRIVATE(flag_save,init,kflag,kop,nfe,relerr_save)
  real ( kind = wp ) s
  real ( kind = wp ) scale
  real ( kind = wp ) t
  real ( kind = wp ) tol
...
subroutine rkf45_s ( f, neqn, y, yp, t, tout, relerr, abserr, flag )
...
!  Advance an approximate solution over one step of length H.
!
      call fehl_s ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, f1 )
      nfe = nfe + 5
!
!  Compute and test allowable tolerances versus local error estimates
!  and remove scaling of tolerances.  The relative error is
...
subroutine error(flag)
  integer :: flag
  if (flag==3) then
     print*,'flag=3: integration was not completed because the input value of RELERR, the & 
          relative error tolerance, was too small.  RELERR has been increased& 
          appropriately for continuing.  If the user accepts the output value of&
          RELERR, then simply reset FLAG to 2 and continue.'
  elseif(flag==4)then
     print*,'flag=4: integration was not completed because more than MAXNFE derivative& 
          evaluations were needed.  This is approximately (MAXNFE/6) steps.&
          The user may continue by simply calling again.  The function counter& 
          will be reset to 0, and another MAXNFE function evaluations are allowed.&'
  elseif (flag==5)then
     print*,'flag=5: integration was not completed because the solution vanished,& 
          making a pure relative error test impossible.  The user must use& 
          a non-zero ABSERR to continue.  Using the one-step integration mode& 
          for one step is a good way to proceed.'

  elseif (flag==6)then
     print*,'flag=6: integration was not completed because the requested accuracy& 
          could not be achieved, even using the smallest allowable stepsize.& 
          The user must increase the error tolerances ABSERR or RELERR before&
          continuing.  It is also necessary to reset FLAG to 2 (or -2 when &
          the one-step integration mode is being used).  The occurrence of &
  ...
  elseif (flag==7)then
     print*,'flag=7: it is likely that this routine is inefficient for solving&
          this problem.  Too much output is restricting the natural stepsize&
          choice.  The user should use the one-step integration mode with& 
          the stepsize determined by the code.  If the user insists upon &
          continuing the integration, reset FLAG to 2 before calling& 
  ...
  elseif (flag==8)then
     print*,'flag=8: invalid input parameters, indicates one of the following:&
          NEQN <= 0;&
          T = TOUT and |FLAG| /= 1;&
          RELERR < 0 or ABSERR < 0;&
          FLAG == 0  or FLAG < -2 or 8 < FLAG.&'
